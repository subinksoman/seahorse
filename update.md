...
[
  ...
  },
  {
    ...
    "effort_days": 90,
    "status": "in_progress",
    "notes": "Commit ab50c2c2d. Phase 0 (hybrid bootstrap + one downgraded service) DONE + verified live: hybrid boots ('[hybrid] Angular 18 bootstrapped ds.lab; helloAngular = wired'), full editor renders (6-node graph, jsPlumb, logo), 0 errors/strictdi. Bundle ~3.5->4.5MB (Angular+zone+JIT compiler; shrinks as AngularJS retires + build moves to AoT). Phase C-1 IN PROGRESS: 22 services migrated to Angular 18 + downgraded. Batches: UserService/UUIDGenerator/TimeService/HelpersService [2a8772c3e/c726b552f/149d86df8]; MouseEvent/EventsService/version [5842ba84e]; ErrorService/NotificationService/MultiSelectionService [8d5135869]; BottomBarService/DragAndDrop/Confirmation+Export+DeleteModalService [b54570ddb]; DefaultInnerWorkflowGenerator/Report/Cluster+LibraryModalService [7bca98630]; the api-client cluster BaseApiClient(base, not downgraded)/OperationsApiClient/WorkflowsApiClient/SessionManagerApi [this batch]. Established the upgraded-provider pattern (ng2/upgraded-providers.ts) — bridges '$rootScope','config','$log','toastr','$timeout','$uibModal','$cookies','$q','$http','ServerCommunication'. api-client cluster design: the legacy BaseApiClient FACTORY returned a constructor extended by two subclass factories; migrated to an Angular base class (BaseApiClient) that the Angular subclasses extend (explicit super() forwarding for JIT-safe DI); base still returns $q promises so consumers' .then/.catch are unchanged (defer HttpClient/RxJS to a later pass). Bottom-up wins compounding (DragAndDrop->MouseEvent, DefaultInnerWorkflowGenerator->UUIDGenerator, Report->BottomBarService, OperationsHierarchyService->OperationsApiClient, WorkflowCloneService->WorkflowsApiClient+NotificationService). Also migrated: OperationsHierarchyService + WorkflowCloneService [1c7a5424b], Operations factory (operations.factory -> ng2/operations.service.ts, injects Angular OperationsApiClient + OperationsHierarchyService) [this batch]. Plus SessionManager [0cee78054] and the WorkflowService HUB (327L, 14 deps — the central editor service: workflow stack, (de)serialization, save-on-change debounce, session lifecycle, inner workflows, cluster, connection validity; 15 consumers) [this batch]. Plus the hub's now-unblocked dependents [batch b0e92b0]: WorkflowStatusBarService (WorkflowService+UserService Angular), NodeCopyPasteVisitorService, CopyPasteService (+ bridged $document/GraphNodesService/CanvasService). Total 30 services. Also scoped tsconfig.json with include/exclude [9c7746490]. Plus the library cluster + editor service [batch cbb7f6f]: LibraryApi(LibraryApiService), LibraryDataConverterService, LibraryService, WorkflowsEditorService — all deps satisfied ($http/$q/$log/config bridged; DeleteModal/Events/Workflow/Operations Angular). Total 35 services (+GraphNodesService [batch f4d803f], which also converts its own bridge to a direct Angular injection in NodeCopyPasteVisitor; bridges DeepsenseNodeParameters; injects NotificationService to fix a latent missing-injection in the notebook-clone error path). common/services/ + common/api-clients/ FULLY Angular. The hub injects 8 migrated Angular services directly + @Injects 6 bridged tokens ($rootScope/$log/ServerCommunication + the still-AngularJS Workflow/DeepsenseCycleAnalyser [deepsense-*] and 3rd-party debounce). NOTE the whole common/api-clients/ cluster is now Angular except LibraryApiService. Detached-method gotcha (fixed 1c7a5424b): legacy closure functions passed as bare callbacks (.then(X.method), $watch(X.getter)) lose `this` as class methods — bind them in the constructor; a full-app scan is the guard (WorkflowService had none). REMAINING (T82 phases 1-5, multi-month): remaining ~16 services + all directives/components/filters. Next: the WorkflowService dependents now fully unblocked (WorkflowStatusBarService, CopyPaste, NodeCopyPasteVisitor), the remaining leaf services, then the component/directive layer via downgradeComponent, ui-router->@angular/router, and the deepsense-* component library + jsPlumb canvas as a late unit, filters->pipes, then leaf components/directives bottom-up, then ui-router->@angular/router + replace the 8 AngularJS-only 3rd-party libs, then the jsPlumb canvas + deepsense-graph-model as one unit, then remove @angular/upgrade+AngularJS+switch to AoT. This is what finally clears the last audit high (angular EOL) + the AngularJS-ecosystem moderates."
  }
]
```
